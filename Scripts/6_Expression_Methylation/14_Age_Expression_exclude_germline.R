### Expression with age by linear regression excluding samples with germline mutations
...
age_DEGs_project <- function(project, germline_patients, purity, bmIDs){
  ...
  df_agg_filtered <- df_agg_filtered[,!(colnames(df_agg_filtered) %in% germline_patients)]
  dim(df_agg_filtered)
  ...
  write.csv(results, paste0("Analysis_results/Gene_Expression/1_Gene_expression_with_age/", project, "_gene_expression_with_age_exclude_germline.csv"), row.names = FALSE)
}

lapply(c("BRCA", "OV"), germline_patients = germline_patients, age_DEGs_project, purity = purity, bmIDs = bmIDs) # because COAD, READ, UCEC also have RNA-seq from Illumina GA


### Gene expression with age in UCEC ##################################################
...
### function to get age-DGEs from cancer
age_DEGs_project_1 <- function(project, germline_patients, purity, bmIDs){
...
age_DEGs_project_1("UCEC", germline_patients = germline_patients ,purity = purity, bmIDs = bmIDs)
