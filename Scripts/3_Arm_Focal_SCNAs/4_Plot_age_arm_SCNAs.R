...
colnames(gain_sig) <- c("cancer_type", "arm", "Regression_coefficient", "std.error",
                        "conf.low", "conf.high", "df.error", "Z", "p.value", "q.value", "Sig", "direction")
write.csv(gain_sig[,c("cancer_type", "arm", "Regression_coefficient", "q.value", "Sig", "direction")], "Source_Data/Fig_2d.csv", row.names = FALSE)
...
colnames(del_sig) <- c("cancer_type", "arm", "Regression_coefficient", "std.error",
                        "conf.low", "conf.high", "df.error", "Z", "p.value", "q.value", "Sig", "direction")
write.csv(del_sig[,c("cancer_type", "arm", "Regression_coefficient", "q.value", "Sig", "direction")], "Source_Data/Fig_2e.csv", row.names = FALSE)


### dot plot for gain
dot_break <- c(min(-log10(gain_sig$q.value)), median(-log10(gain_sig$q.value)), max(-log10(gain_sig$q.value)))

pdf("Analysis_results/CNAs/3_Age_biases_recurrent_SCNAs/Age_recurrent_arm_new/Multivariate_age_recurrent_arm_gain.pdf", width = 8, height = 4, useDingbats=FALSE) 
p <- ggplot(data = gain_sig, aes(x = arm, y = cancer_type)) +
  ...
  scale_fill_gradient2(name = "Regression coefficient", mid = "#bdbdbd", midpoint = 0, low = "#1D91C0", high = "#a50f15", guide = "colourbar") +
  scale_size_continuous(range = c(2, 5), breaks = dot_break, labels = format(round(dot_break, 2), nsmall = 2)) +
  ggtitle("Age and arm-level copy-number gain") +
...
pdf("Analysis_results/CNAs/3_Age_biases_recurrent_SCNAs/Age_recurrent_arm_new/Multivariate_age_recurrent_arm_del.pdf", width = 8, height = 4, useDingbats=FALSE) 
p <- ggplot(data = del_sig, aes(x = arm, y = cancer_type)) +
  ...
  scale_fill_gradient2(name = "Regression coefficient", mid = "#bdbdbd", midpoint = 0, low = "#1D91C0", high = "#a50f15", guide = "colourbar") +
  scale_size_continuous(range = c(2, 5), breaks = dot_break, labels = format(round(dot_break, 2), nsmall = 2)) +
  ggtitle("Age and arm-level copy-number loss") +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
        axis.text.x = element_text(size = 15, angle = 90, vjust = 0.5, hjust = 1),
        axis.text.y = element_text(size = 15),
        #axis.title.x = element_text(size=15,face="bold"),
