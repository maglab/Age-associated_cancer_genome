### Age biases in focal-level recurrent SCNAs (multiple logistic regression) removing patients with SCNA in normal samples (from Tom)
...
samples_to_exclude <- as.character(unlist(read.table("Data/58_to_exclude.txt")))
clean_samples <- function(id){
  tmp <- unlist(strsplit(id, split = "-"))[1:3]
  return(paste0(tmp, collapse = "-"))
}
samples_to_exclude <- unlist(lapply(samples_to_exclude, clean_samples))
...
test_age_gain <- function(Unique.Name, project, purity, samples_to_exclude){
  ...
  # exclude patients with SCNA in normal samples
  df <- df[!(df$patient %in% samples_to_exclude), ]
  ...
  write.csv(result, paste0("Analysis_results/CNAs/3_Age_biases_recurrent_SCNAs/Age_recurrent_focal_new/Multivariate_age_recurrent_focal_gain_exclude_normal_SCNAs/", project, "_multivariate_age_SCNA_", Name, "_exclude_normal_SCNAs.csv", collapse = ""), row.names = FALSE)
  ...
  colnames(result_df) <- c("region", "estimate", "std.error", "conf.low", "conf.high", "statistic", "df.error", "p.value", "cancer_type", "Unique.Name")
  result_df <- result_df[,c("cancer_type", "Unique.Name", "region", "estimate", "std.error", "conf.low", "conf.high", "statistic", "df.error", "p.value")]
...
for(i in 1:nrow(gain_df)){
  ...
  tmp <- test_age_gain(Unique.Name = Unique.Name, project = project, purity = purity, samples_to_exclude = samples_to_exclude)
...
write.csv(result_df, "Analysis_results/CNAs/3_Age_biases_recurrent_SCNAs/Age_recurrent_focal_new/Multivariate_age_recurrent_focal_gain_exclude_normal_SCNAs.csv", row.names = FALSE)
...
write.csv(result_df, "Analysis_results/CNAs/3_Age_biases_recurrent_SCNAs/Age_recurrent_focal_new/Multivariate_age_recurrent_focal_gain_exclude_normal_SCNAs_sig.csv", row.names = FALSE)
